...
se asume que los patrones de comportamiento de consumo son universales y transferibles al contexto de La Paz. 
	...
	estado \emph{ready} sin asignación de courier.  Los valores reportados corresponden
	al promedio de las siete réplicas válidas salvo que se indique lo contrario.
	...
	\midrule
	S1 & 10.09 & 32.33 & $+220.3$ & 13.10 & 49.10 & $+274.8$ & 100.0 & 76.0 & $-24.0$ \\
	S2 & 16.68 & 36.26 & $+117.4$ & 32.73 & 56.27 & $+71.9$  & 91.7  & 68.9 & $-22.7$ \\
	S3 & 35.79 & 44.10 & $+23.2$  & 117.19 & 72.36 & $-38.3$ & 76.3  & 61.0 & $-15.2$ \\
	S4 & 75.41 & 62.40 & $-17.3$  & 228.01 & 157.15 & $-31.1$ & 61.2  & 50.9 & $-10.3$ \\
	\bottomrule
	\end{tabular}
	\end{table}
	...
	holgada, FCFS logra tiempos de entrega sustancialmente menores que RH
	(10.09 vs 32.33~min en S1; 16.68 vs 36.26~min en S2).  Esto se explica porque,
	con exceso de repartidores, FCFS despacha cada pedido inmediatamente al courier
	más cercano, mientras que RH introduce latencia al esperar la formación de
	agrupamientos.  Aunque ambas políticas alcanzan tiempos bajos en términos absolutos,
	la diferencia relativa es considerable ($+220\%$ en S1).
	...
	ponderada.  En S4, RH reduce el CtD promedio un 17.3\% (62.40 vs 75.41~min)
	y el P90~CtD un 31.1\% (157.15 vs 228.01~min).  La reducción del P90~CtD es
	particularmente relevante, ya que refleja la mejora en el peor caso percibido por
	los clientes.
	...
	\midrule
	S1 & 0.14    & 15.41 & --      & 0.00  & 2.42  & --       \\
	S2 & 5.73    & 18.87 & $+229.2$ & 2.84  & 5.52  & $+94.0$  \\
	S3 & 23.91   & 26.20 & $+9.6$   & 17.24 & 12.55 & $-27.2$  \\
	S4 & 62.55   & 44.03 & $-29.6$  & 52.06 & 29.56 & $-43.2$  \\
	\bottomrule
	\end{tabular}
	\end{table}
	...
	del pedido.  Bajo RH, el mecanismo de lookahead
	($\Delta_1 = 20$~min) agrupa pedidos antes de despachar, lo que incrementa el RtP
	a 15.41~min en S1 y 18.87~min en S2.  A medida que crece la saturación, el RtP
	bajo FCFS aumenta considerablemente (62.55~min en S4), mientras que RH logra
	menor RtP en el escenario más saturado (44.03~min, $-29.6\%$).
	...
	Esc. & FCFS & RH & $\Delta$\% & FCFS & RH & $\Delta$\% & FCFS & RH & $\Delta$\% \\
	\midrule
	S1 & 1.43 & 4.07 & $+183.5$ & 18.0 & 37.1 & $+106.3$ & 4.49 & 3.11 & $-30.6$ \\
	S2 & 2.07 & 5.25 & $+154.4$ & 27.3 & 43.8 & $+60.1$  & 4.77 & 2.82 & $-40.9$ \\
	S3 & 2.77 & 6.62 & $+138.9$ & 38.4 & 50.0 & $+30.4$  & 5.04 & 2.55 & $-49.4$ \\
	S4 & 3.62 & 8.29 & $+129.1$ & 53.0 & 56.6 & $+6.8$   & 5.41 & 2.29 & $-57.7$ \\
	\bottomrule
	\end{tabular}
	\end{table}
	...
	El indicador Orders/Courier/Hr mide la productividad individual.  Bajo la
	política RH, el throughput es consistentemente superior en todos los escenarios,
	desde $+183.5\%$ en S1 hasta $+129.1\%$ en S4.  Esta ganancia se explica por
	el efecto del agrupamiento: al asignar múltiples pedidos a un mismo courier por
	viaje, cada hora de trabajo activo produce más entregas.  El throughput crece con
	la saturación bajo ambas políticas, ya que con menos repartidores cada uno debe
	atender más pedidos.

	La utilización (\% del turno en conducción) crece con la saturación bajo ambas
	políticas.  Bajo RH, la utilización es consistentemente mayor ($+106.3\%$ en S1
	a $+6.8\%$ en S4), porque los agrupamientos generan rutas multi-parada
	más largas, manteniendo al courier en movimiento durante una fracción mayor del
	turno.

	\subsection{Distancia y enrutamiento}

	La distancia por pedido refleja la eficiencia geográfica del enrutamiento.
	Bajo RH, la distancia por pedido es consistentemente menor en todos los escenarios
	($-30.6\%$ en S1 a $-57.7\%$ en S4), lo cual es consecuencia directa de los
	agrupamientos: las rutas multi-parada reparten la distancia total del viaje entre
	varios pedidos, reduciendo el costo de transporte por unidad.  La ventaja se
	acentúa con la saturación, ya que la compresión geográfica de los agrupamientos
	es más eficaz con flotas reducidas.
	...
	Esc. & FCFS & RH & $\Delta$\% & FCFS & RH & $\Delta$\% & FCFS & RH & $\Delta$\% \\
	\midrule
	S1 & 1.00 & 2.75 & $+174.6$ & 0.0 & 71.1 & $+71.1$ & 11.40 & 10.00 & $-12.2$ \\
	S2 & 1.00 & 3.10 & $+209.6$ & 0.0 & 82.4 & $+82.4$ & 10.01 & 10.00 & $-0.1$  \\
	S3 & 1.00 & 3.34 & $+234.0$ & 0.0 & 90.6 & $+90.6$ & 10.00 & 10.00 & $+0.0$  \\
	S4 & 1.00 & 3.60 & $+259.7$ & 0.0 & 95.9 & $+95.9$ & 10.00 & 10.00 & $+0.0$  \\
	\bottomrule
	\end{tabular}
	\end{table}
	...
	Click-to-Door en cada escenario.  Para cada escenario se reporta la media de la
	correlación rank-biserial ($\bar{r}$) y su desviación estándar a través de las
	siete semillas válidas; todas las pruebas individuales resultaron significativas
	tras la corrección de Benjamini-Hochberg por comparaciones múltiples.
	...
	\midrule
	S1 & 10.10 & 32.33 & $<0.001$ & $0.985 \pm 0.004$ & Grande \\
	S2 & 16.68 & 36.27 & $<0.001$ & $0.790 \pm 0.043$ & Grande \\
	S3 & 35.79 & 44.10 & $<0.001$ & $0.514 \pm 0.043$ & Grande \\
	S4 & 75.41 & 62.40 & $<0.001$ & $0.264 \pm 0.030$ & Peq.   \\
	\bottomrule
	\end{tabular}
	\end{table}

	La prueba de Kruskal-Wallis sobre las diferencias CtD(RH) $-$ CtD(FCFS)
	confirma que el efecto de la política varía significativamente entre escenarios
	($H = 515.67$, $p < 0.001$), lo que justifica el análisis por regímenes.
	...
	(10.10 vs 32.33~min en S1; 16.68 vs 36.27~min en S2).
	La prueba de Mann-Whitney confirma diferencias significativas con tamaño de efecto
	grande ($\bar{r} = 0.985$ y $\bar{r} = 0.790$, respectivamente), \emph{a favor de FCFS}.
	...
	esperar la formación de agrupamientos.  En este régimen, la coordinación
	adicional del RH no aporta ventaja marginal en velocidad de entrega.

	\paragraph{Régimen de saturación media (S3).}
	Con 30~repartidores, los tiempos medios de CtD difieren moderadamente
	(35.79 vs 44.10~min), pero la distribución presenta cambios notables.
	El P90~CtD bajo FCFS alcanza 117.19~min frente a 72.36~min bajo RH
	($-38.3\%$), y el CTD Overage se reduce de 17.24 a 12.55~min ($-27.2\%$).
	La prueba de Mann-Whitney detecta un efecto grande ($\bar{r} = 0.514$),
	...
	Con 20~repartidores para $\sim$1\,026~órdenes, RH reduce el CtD promedio un
	17.3\% (62.40 vs 75.41~min) y el P90~CtD un 31.1\%.  Lo más notable es la
	reducción de pedidos no entregados: 2.0\% bajo RH frente a 11.5\% bajo FCFS.
	El tamaño de efecto de la prueba Mann-Whitney es pequeño ($\bar{r} = 0.264$),
	...
	\begin{itemize}
		\item Tres de las diez semillas (2027, 2033, 2034) no generaron resultados
		      ...
		      sin asignación de courier.  Aunque las siete semillas restantes proporcionan
		      poder estadístico suficiente, este fallo sugiere una condición de borde
		      en el simulador que merece investigación futura.
	...
	producen instancias reproducibles de $\sim$1\,026~órdenes distribuidas en
	25~restaurantes geolocalizados; siete de ellas generaron resultados válidos.

	\paragraph{OE\,2: Integración OSRM--simulador.}
	El motor de simulación de eventos discretos se integró con OSRM v5.26 sobre
	cartografía OpenStreetMap, utilizando el algoritmo CH (Contraction Hierarchies)
	...
	obteniendo $p < 0.001$ en las 28~comparaciones individuales (7~semillas $\times$
	4~escenarios).  La correlación rank-biserial promedio
	varía de $\bar{r} = 0.985$ (Grande) en S1 a $\bar{r} = 0.264$ (Pequeño) en S4,
	confirmando diferencias estadísticamente significativas cuya dirección e
	intensidad dependen del nivel de saturación.  La prueba ómnibus de
	Kruskal-Wallis ($H = 515.67$, $p < 0.001$) confirma que el efecto varía
	significativamente entre escenarios.
	...
	\begin{itemize}
		\item \textbf{Baja saturación (S1--S2).}  FCFS es superior en CtD promedio
		      (10.10 vs 32.33~min en S1; 16.68 vs 36.27~min en S2) porque despacha
		      cada pedido inmediatamente sin latencia de agrupamiento.  En este
		      régimen, la hipótesis no se sostiene para la métrica CtD.
		\item \textbf{Saturación media (S3).}  El CtD promedio difiere moderadamente
		      (35.79 vs 44.10~min), pero RH comprime la cola derecha: P90~CtD
		      $-38.3\%$ y CTD~Overage $-27.2\%$.  La hipótesis se sostiene
		      para indicadores de robustez del servicio.
		\item \textbf{Alta saturación (S4).}  RH reduce el CtD $-17.3\%$,
		      el P90~CtD $-31.1\%$ y los pedidos no entregados de 11.5\% a 2.0\%.
		      La hipótesis se confirma plenamente en este régimen.
	\end{itemize}

	En cuanto a la eficiencia de la flota, el throughput bajo RH es consistentemente
	superior en todos los escenarios ($+129\%$ a $+184\%$), gracias al efecto del
	agrupamiento.  La utilización crece bajo RH ($+106\%$ en S1 a $+6.8\%$ en S4),
	lo que indica un uso más intensivo de los recursos de transporte.

...
"órdenes por hora" y nivel de servicio "\textit{Ready-to-Door"} a la estrategia reactiva estándar (FCFS con estimación
...
implementa una instancia local del servidor de enrutamiento \textbf{OSRM v5.26.0} que opera sobre 
...
\begin{itemize}
	...
	\item \textbf{Infraestructura de Enrutamiento Contenerizada:} El motor de enrutamiento OSRM se despliega mediante \textbf{Docker} (imagen \texttt{osrm/osrm-backend}), 
...
se asume que los patrones de comportamiento de consumo son universales y transferibles al contexto de La Paz. 
	...
	proponen \textbf{Contraction Hierarchies} (CH), una técnica de
	...
	métricas de evaluación. Cada sección establece la correspondencia entre los conceptos teóricos del Capítulo~2 
	y su realización en código.
	...
	\begin{figure}[htbp]
		...
		% CAPA 1: ENRUTAMIENTO
		% =============================================
		\node[box=2.6cm] (osm) at (0, 0) {map.osm\\\scriptsize(OSM .pbf)};
		\node[box=3.6cm] (chfiles) at (7, 0.3)
			{bcs-latest.osrm\\\scriptsize(CH)\\\scriptsize 28 archivos indice};
		\node[box=3.6cm] (routed) at (7, -2.2)
			{osrm-routed :5000\\\scriptsize API REST (CH)};

		\draw[arr] (osm) -- node[above, font=\scriptsize, align=center]
			{osrm-extract\\osrm-contract} (chfiles);
		\draw[arr] (chfiles) -- (routed);

		\node[font=\scriptsize, anchor=north east, text=gray, align=right]
			at ([xshift=-0.4cm, yshift=0.2cm]routed.west)
			{Docker (5.26.0)\\docker-compose.yml};
		...
		% =============================================
		% ETIQUETA CONEXION CAPA 1 -> CAPA 2
		% =============================================
		\draw[arr, dashed] (routed.south) -- ++(0,-1.0)
			node[right, font=\scriptsize, align=left, xshift=2pt]
			{HTTP GET\\/route/v1/driving/} -- ++(0,-0.8);

		% =============================================
		% CAPA 2: SIMULACION
		% =============================================
		\node[box=2.6cm] (synth) at (0, -7) {synth\_loader.py};
		\node[box=3cm]   (main)  at (4, -7)
			{main.py\\\scriptsize(epocas $f{=}5$)};
		\node[box=3.6cm] (route) at (8.8, -7)
			{getrouteOSMR.py\\\scriptsize(cliente HTTP +\\cache en memoria)};

		\node[box=1.8cm, minimum height=0.6cm] (pol) at (4, -9)
			{FCFS / RH};

		\node[box=2.6cm] (config) at (0, -10.5)
			{config.py\\\scriptsize(params)};
		\node[box=3.6cm] (csv) at (8.8, -10.5)
			{resultados CSV\\\scriptsize + mapas HTML};

		\draw[arr]  (synth) -- (main);
		\draw[darr] (main)  -- (route);
		\draw[arr]  (config.north) -- ++(0,0.5) -| (main.south west);
		\draw[arr]  (pol.east) -| (csv);

		% flecha de capa 1 a route (llega por arriba)
		\draw[arr, dashed] ([yshift=-2.8cm]routed.south) -- (route.north);

		\begin{scope}[on background layer]
			\node[layer=green!40, fit=(synth)(main)(route)(config)(csv)(pol)] (L2) {};
		\end{scope}
		\node[layertitle, anchor=south] at (L2.north) {Capa de Simulacion (Python)};

		% =============================================
		% CONEXION CAPA 2 -> CAPA 3
		% =============================================
		\draw[arr, dashed] (csv.south) -- ++(0,-1.2) -| (3.5,-14);

		% =============================================
		% CAPA 3: ANALISIS
		% =============================================
		\node[box=3.4cm] (genr) at (2.5, -14.8)
			{generate\_results.py\\\scriptsize(21 KPIs por politica)};
		\node[box=4cm] (runx) at (8, -14.8)
			{run\_experiments.py\\\scriptsize(orquestador multi-\\escenario de saturacion)};

		\begin{scope}[on background layer]
			\node[layer=orange!40, fit=(genr)(runx)] (L3) {};
		\end{scope}
		\node[layertitle, anchor=south] at (L3.north) {Capa de Analisis};

		\end{tikzpicture}
		\caption{Arquitectura de tres capas del entorno de simulacion MDRP-BCS.}
	...
	Se implementa mediante OSRM (\textit{Open Source Routing Machine}) v5.26.0, desplegado como un contenedor 
	Docker orquestado por \texttt{docker-compose}.
	...
	\begin{enumerate}
		\item \textbf{Extracción} (\texttt{osrm-extract}): Parsea el archivo \texttt{.pbf} y genera una 
		representación intermedia del grafo vial, aplicando el perfil de velocidades del vehículo 
		...
		\item \textbf{Contracción} (\texttt{osrm-contract}): Construye la jerarquía de Contraction 
		Hierarchies (CH) \citep{Geisberger2008} sobre el grafo extraído. El algoritmo asigna un 
		\textit{orden de importancia} a cada nodo y genera atajos (\textit{shortcuts}) que permiten 
		saltar nodos intermedios durante las consultas. El resultado son 28 archivos de índice 
		(\texttt{bcs-latest.osrm.*}) que codifican la jerarquía completa.
	\end{enumerate}
	...
	y el archivo \texttt{.pbf} original de $\sim$30 MB).
	...
	nuevas calles, colonias y modificaciones viales mediante contribuciones colaborativas, los archivos 
	...
	\begin{itemize}
		\item \textbf{Aislamiento del entorno:} La imagen Docker \texttt{osrm/osrm-backend:5.26.0} fija 
	...
	\begin{itemize}
		...
		\item \textbf{Resiliencia:} El cliente HTTP utiliza una sesión persistente (\texttt{requests.Session}) 
		con política de reintentos configurada: hasta 5 reintentos con \textit{backoff} exponencial para 
		códigos de error 429 (rate limit) y 5xx (errores del servidor). Esto previene fallos transitorios 
		durante las simulaciones largas.
		
		\item \textbf{Caché en memoria:} Los resultados de rutas previamente consultadas se almacenan en un 
		diccionario en memoria (\texttt{\_osrm\_cache}), indexado por la tupla de coordenadas. Dado que los 
		restaurantes tienen ubicaciones fijas y las rutas se repiten con frecuencia durante la asignación de 
		agrupamientos, esta caché reduce significativamente el número de consultas HTTP al servidor OSRM.
		
		\item \textbf{Fallback euclidiano:} Si la consulta OSRM falla (por desconexión o error de red), el 
		...
		interrumpa, aunque los resultados con fallback euclidiano se señalan en la salida para que el 
		investigador los detecte.
	\end{itemize}
	...
	\texttt{main.py}                & Motor de simulación por épocas y modelos de datos \\
	\texttt{getrouteOSMR.py}        & Cliente HTTP para OSRM con caché y fallback \\
	...
	\texttt{config.py}, que controlan el comportamiento de la simulación y permiten la experimentación 
	paramétrica sin modificar el código fuente.
	...
	Horizonte de asignación        & $\Delta_u$    & 20 min & Lookahead para órdenes \\
	...
	Lookahead de ready time        & $\Delta_1$    & 20 min & Ventana de anticipación \\
	Lookahead de disponibilidad    & $\Delta_2$    & 20 min & Ventana para couriers \\
	...
	\subsection{Capa de análisis: pipeline de resultados}
	...
	\begin{itemize}
		\item \texttt{generate\_results.py}: Orquesta el pipeline completo para un escenario individual. 
		Ejecuta secuencialmente la generación de datos sintéticos, la simulación FCFS, la simulación 
		Rolling Horizon, y el cálculo de 21 KPIs comparativos. Los resultados se consolidan en 
		\texttt{results/kpi\_comparison.csv}.
		
		\item \texttt{run\_experiments.py}: Amplía el pipeline anterior para ejecutar múltiples escenarios 
		de saturación de forma automatizada. Define cuatro niveles de relación órdenes/courier 
		(18:1, 26:1, 35:1 y 52:1), ejecuta ambas políticas en cada escenario, y genera una tabla 
		consolidada (\texttt{experiment\_comparison.csv}) que reúne los KPIs de todos los escenarios. 
		Adicionalmente, soporta la ejecución con múltiples semillas aleatorias (\texttt{-{}-seeds}) para 
		análisis de robustez estadística y la variación de parámetros algorítmicos 
		(\texttt{-{}-bundle-sizes}).
	\end{itemize}
	...
	Dado que no existen registros históricos de operaciones de entrega de comida en La Paz, B.C.S., 
	...
	\subsection{Generación temporal de órdenes: proceso Poisson no homogéneo}

	La llegada de órdenes se modela mediante un \textbf{proceso de Poisson no homogéneo} 
	\citep{Ross2014} con tasa variable $\lambda(t)$, siguiendo la evidencia empírica del dataset 
	LaDe \citep{Wu2023LaDe}. La tasa de llegada se define como la superposición de una intensidad 
	base constante y dos picos gaussianos que capturan los horarios de mayor demanda:

	\begin{equation}
		\lambda(t) = \lambda_0 + A_m \cdot \exp\!\left(-\frac{(t - \mu_m)^2}{2\sigma^2}\right) 
		+ A_e \cdot \exp\!\left(-\frac{(t - \mu_e)^2}{2\sigma^2}\right)
		\label{eq:arrival_rate}
	\end{equation}

	\noindent donde $\lambda_0 = 0.6$ órdenes/minuto es la intensidad base, $\mu_m = \text{09:00}$ y 
	$\mu_e = \text{17:00}$ son los centros de los picos matutino y vespertino respectivamente, 
	$A_m = 2.4$ y $A_e = 2.0$ son las amplitudes de cada pico, y $\sigma = 45$~min controla la 
	dispersión temporal de la demanda. Los valores de $\mu_m$ y $\mu_e$ se calibraron a partir de 
	los patrones observados en el dataset LaDe \citep{Wu2023LaDe}, que documenta picos de demanda 
	análogos en servicios de entrega de comida en ciudades chinas.

	El perfil de intensidad se discretiza en intervalos de un minuto sobre la ventana operativa de 
	11 horas (08:00--19:00, $T = 660$ min). Para cada minuto $t$, el número de órdenes generadas 
	se muestrea de una distribución de Poisson con parámetro $\lambda(t)$:
	...
	estará lista para ser recogida por el courier. Este tiempo se modela como una variable aleatoria 
	normal truncada:
	...
	la distancia mínima de 2~km como mecanismo de fallback.
	...
	Estas coordenadas reales se reproyectan dentro del polígono urbano $\mathcal{P}$ para 
	mantener la consistencia con la zona de operación delimitada.
	...
	\begin{enumerate}
		...
		\item Si la solicitud falla, aplica el fallback euclidiano (Haversine a 320~m/min).
	...
	Este capítulo presenta los resultados obtenidos de la ejecución del simulador bajo cinco
	...
	Cada escenario se ejecuta con dos semillas aleatorias ($s \in \{2025, 2026\}$)
	para garantizar la robustez estadística de los resultados; los valores reportados
	corresponden al promedio de las dos réplicas salvo que se indique lo contrario.

	\section{Configuración experimental}

	La Tabla~\ref{tab:escenarios} resume los cinco escenarios de saturación.  En todos los
	casos se utiliza la misma instancia base de La Paz, B.C.S., con aproximadamente 1\,038
	órdenes generadas sintéticamente (véase §3.2) y 25 restaurantes.  La variable
	...
	$B_{max} = 4$.  El enrutamiento se realiza mediante OSRM v5.26.0 con el algoritmo
	Contraction Hierarchies sobre el grafo vial de La Paz.

	Para cada escenario y semilla se ejecutan secuencialmente las políticas FCFS y RH,
	generando en total $4 \times 2 \times 2 = 16$ corridas de simulación.  Los 21 KPIs
	...
	sobre las dos réplicas.  El Click-to-Door (CtD) es la métrica principal de calidad
	...
	\centering
	\small
	\caption{Indicadores de calidad de servicio: promedio de 2 réplicas.}
	\label{tab:res_ctd}
	...
	\midrule
	S1 & 10.05 & 29.59 & $+194.4$ & 13.09 & 44.03 & $+236.4$ & 100.0 & 84.3 & $-15.7$ \\
	S2 & 17.03 & 32.79 & $+92.5$  & 32.65 & 50.46 & $+54.5$  & 91.4  & 76.7 & $-16.1$ \\
	S3 & 37.11 & 37.82 & $+1.9$   & 123.98 & 58.15 & $-53.1$ & 76.3  & 68.8 & $-9.8$  \\
	S4 & 78.57 & 57.93 & $-26.3$  & 246.42 & 141.30 & $-42.7$ & 61.3  & 55.5 & $-9.5$ \\
	\bottomrule
	\end{tabular}
	\end{table}
	...
	En los escenarios de baja saturación (S1--S2), donde la oferta de repartidores es
	holgada, ambas políticas alcanzan tiempos de entrega bajos.  Sin embargo, el CtD bajo
	FCFS incluye un componente de espera del courier en el restaurante: el repartidor
	llega antes de que el pedido esté listo, produciendo valores negativos de
	Ready-to-Pickup (RtP).  Aunque esto reduce el CtD aparente, no representa una
	ganancia real de eficiencia operativa.

	A medida que aumenta la saturación (S3--S4), la heurística RH muestra su ventaja al
	coordinar la formación de agrupamientos y la asignación mediante la función de costo
	ponderada.  La reducción del P90~CtD es particularmente relevante, ya que refleja la
	mejora en el peor caso percibido por los clientes.
	...
	courier al restaurante.

	\begin{table}[htbp]
	\centering
	\small
	\caption{Ready-to-Pickup y CTD Overage: promedio de 2 réplicas.}
	\label{tab:res_rtp}
	...
	\midrule
	S1 & $-9.45$ & 8.03 & -- & 0.00 & 1.26 & -- \\
	S2 & $-2.95$ & 10.14 & -- & 3.23 & 3.52 & $+9.2$ \\
	S3 & 16.71 & 14.37 & $-14.0$ & 18.72 & 7.42 & $-60.4$ \\
	S4 & 57.64 & 33.75 & $-41.5$ & 55.50 & 25.92 & $-53.3$ \\
	\bottomrule
	\end{tabular}
	\end{table}

	Los valores negativos de RtP bajo FCFS en escenarios de baja saturación indican que
	el courier llega al restaurante antes de que el pedido esté preparado; el repartidor
	debe esperar en el establecimiento.  Bajo la política RH, el mecanismo de lookahead
	($\Delta_1 = 20$~min) sincroniza la llegada del courier con la disponibilidad del
	pedido, eliminando o reduciendo esta espera improductiva.

	El CTD Overage captura el exceso promedio sobre el objetivo SLA de $\tau = 40$~min.
	Un overage de cero indica que todos los pedidos se entregan dentro del estándar de
	calidad.
	...
	\centering
	\small
	\caption{Indicadores de eficiencia operativa: promedio de 2 réplicas.}
	...
	Esc. & FCFS & RH & $\Delta$\% & FCFS & RH & $\Delta$\% & FCFS & RH & $\Delta$\% \\
	\midrule
	S1 & 1.63 & 1.63 & $0.0$   & 20.4 & 37.5 & $+83.6$ & 4.44 & 7.97 & $+79.6$ \\
	S2 & 2.36 & 2.36 & $0.0$   & 31.5 & 46.2 & $+46.5$ & 4.79 & 6.69 & $+39.9$ \\
	S3 & 3.15 & 3.15 & $0.0$   & 44.2 & 53.3 & $+20.6$ & 5.09 & 5.73 & $+12.6$ \\
	S4 & 4.14 & 4.67 & $+12.8$ & 61.7 & 63.8 & $+3.5$  & 5.49 & 4.60 & $-16.1$ \\
	\bottomrule
	\end{tabular}
	\end{table}
	...
	El indicador Orders/Courier/Hr mide la productividad individual.  Bajo ambas
	políticas, este indicador crece con la saturación: con menos repartidores, cada uno
	debe atender más pedidos.  En los escenarios S1--S3, ambas políticas entregan la
	totalidad de las órdenes, por lo que el throughput es idéntico.  En S4, la
	política RH logra un throughput 12.8\% superior (4.67 vs 4.14~ord./c/hr) gracias a
	la reducción de pedidos no entregados (1.2\% vs 12.4\%).

	La utilización (\% del turno en conducción) crece con la saturación bajo ambas
	políticas, alcanzando 61.7\% (FCFS) y 63.8\% (RH) en S4.  Bajo RH, la utilización
	es consistentemente mayor en todos los escenarios porque los agrupamientos generan
	rutas multi-parada más largas, manteniendo al courier en movimiento durante una
	fracción mayor del turno.

	\subsection{Distancia y enrutamiento}

	La distancia por pedido refleja la eficiencia geográfica del enrutamiento.
	En los escenarios S1--S3, la distancia por pedido es mayor bajo RH (+12.6\% a
	+79.6\%), lo cual es consecuencia directa de los agrupamientos: las rutas
	multi-parada recorren mayor distancia en total, aunque reparten dicha distancia
	entre varios pedidos.  En S4, la tendencia se invierte: RH logra una distancia
	por pedido 16.1\% menor (4.60~vs~5.49~km), porque al consolidar entregas evita
	viajes redundantes en un entorno donde los repartidores están altamente saturados.
	...
	\centering
	\small
	\caption{Indicadores de agrupamiento y costos: promedio de 2 réplicas.}
	\label{tab:res_bundles}
	\begin{tabular}{@{}l rr r rr r rr r@{}}
	\toprule
	& \multicolumn{3}{c}{Avg Bundle} & \multicolumn{3}{c}{\% Multi} & \multicolumn{3}{c}{Costo/Ped. (\$)} \\
	\cmidrule(lr){2-4} \cmidrule(lr){5-7} \cmidrule(lr){8-10}
	Esc. & FCFS & RH & $\Delta$\% & FCFS & RH & $\Delta$\% & FCFS & RH & $\Delta$\% \\
	\midrule
	S1 & 1.00 & 2.56 & $+156.4$ & 0.0 & 66.5 & $+66.5$ & 10.49 & 10.38 & $-1.1$ \\
	S2 & 1.00 & 2.99 & $+198.8$ & 0.0 & 78.9 & $+78.9$ & 9.89  & 9.87  & $-0.3$ \\
	S3 & 1.00 & 3.30 & $+230.1$ & 0.0 & 89.4 & $+89.4$ & 10.00 & 10.00 & $0.0$  \\
	S4 & 1.00 & 3.57 & $+256.5$ & 0.0 & 95.4 & $+95.4$ & 9.82  & 9.84  & $+0.2$ \\
	\bottomrule
	\end{tabular}
	\end{table}
	...
	por lo que el tamaño promedio de agrupamiento es siempre 1.0 y el porcentaje de
	pedidos en agrupamientos múltiples es 0\%.  Bajo RH, el módulo de \emph{bundling}
	genera agrupamientos de hasta $B_{max} = 4$ órdenes por restaurante, permitiendo
	que un viaje atienda múltiples entregas.

	La Figura~\ref{fig:bundle_dist} muestra la distribución del tamaño de agrupamiento
	bajo RH en los cuatro escenarios.
	...
	$p_2 = \$15$/hora de turno.  El costo por pedido ($\text{Comp.\ total} / n_{\text{entregados}}$)
	refleja la eficiencia económica de la operación.  Bajo condiciones de baja saturación,
	muchos couriers no alcanzan el umbral variable y reciben el pago mínimo garantizado,
	elevando el costo unitario.  A medida que aumenta la saturación, el pago variable
	domina y el costo por pedido converge hacia $p_1$.

	La fracción de repartidores que recibe el pago mínimo (Frac.~Mín.) es un indicador
	del sobredimensionamiento de la flota: valores altos señalan repartidores ociosos cuyo
	costo se absorbe sin generar entregas adicionales.
	...
	Mann-Whitney $U$~\citep{MannWhitney1947} a cuatro métricas a nivel de orden:
	Click-to-Door, Ready-to-Pickup, Ready-to-Door y tamaño de agrupamiento.
	Se utiliza un nivel de significancia $\alpha = 0{.}05$ y se reporta la correlación
	...
	de los KPIs sean normales, condición que rara vez se cumple en tiempos de entrega;
	...
	Click-to-Door en cada escenario, con los datos agregados de las dos réplicas.

	\begin{table}[htbp]
	\centering
	\small
	\caption{Prueba de Mann-Whitney $U$ para Click-to-Door: 2 réplicas agregadas.}
	\label{tab:mann_whitney}
	\begin{tabular}{@{}l rr cc cc@{}}
	\toprule
	Esc. & $\bar{x}_{\text{FCFS}}$ & $\bar{x}_{\text{RH}}$ & $U$ & $p$ & $r$ & Efecto \\
	\midrule
	S1 & 10.05 & 29.59 & 44\,784    & $<0.001$ & 0.979 & Grande \\
	S2 & 17.03 & 32.79 & 487\,834   & $<0.001$ & 0.774 & Grande \\
	S3 & 37.11 & 37.82 & 1\,102\,428 & $<0.001$ & 0.490 & Med.   \\
	S4 & 78.57 & 57.93 & 1\,395\,659 & $<0.001$ & 0.255 & Peq.   \\
	\bottomrule
	\end{tabular}
	\end{table}
	...
	Los resultados revelan tres regímenes de comportamiento en función de la saturación.

	\paragraph{Régimen de baja saturación (S1--S2).}
	Con una flota holgada, FCFS logra tiempos CtD sustancialmente menores que RH
	(10.05 vs 29.59~min en S1; 17.03 vs 32.79~min en S2).
	La prueba de Mann-Whitney confirma diferencias significativas con tamaño de efecto
	grande ($r = 0.979$ y $r = 0.774$, respectivamente), \emph{a favor de FCFS}.
	Esto se explica porque, con exceso de repartidores, FCFS despacha cada pedido
	inmediatamente al courier más cercano, mientras que RH introduce latencia al
	esperar la formación de agrupamientos.  El componente negativo de RtP bajo FCFS
	($-9.45$ y $-2.95$~min) refleja couriers que llegan al restaurante antes de que
	el pedido esté listo, reduciendo el CtD aparente.  En este régimen, la
	coordinación adicional del RH no aporta ventaja marginal en velocidad de entrega.

	\paragraph{Régimen de saturación media (S3).}
	Con 30~repartidores, los tiempos medios de CtD son prácticamente equivalentes
	(37.11 vs 37.82~min), pero la distribución difiere drásticamente.
	El P90~CtD bajo FCFS alcanza 123.98~min frente a 58.15~min bajo RH
	($-53.1\%$), y el CTD Overage se reduce de 18.72 a 7.42~min ($-60.4\%$).
	La prueba de Mann-Whitney detecta un efecto mediano ($r = 0.490$),
	indicando que la heurística RH comprime la cola derecha de la distribución
	mediante la coordinación de agrupamientos y la sincronización de tiempos de
	recogida.

	\paragraph{Régimen de alta saturación (S4).}
	Con 20~repartidores para $\sim$1\,040~órdenes, RH reduce el CtD promedio un
	26.3\% (57.93 vs 78.57~min) y el P90~CtD un 42.7\%.  Lo más notable es la
	reducción de pedidos no entregados: 1.2\% bajo RH frente a 12.4\% bajo FCFS.
	El tamaño de efecto de la prueba Mann-Whitney es pequeño ($r = 0.255$),
	lo cual refleja la alta variabilidad de ambas distribuciones en condiciones
	extremadamente saturadas, no la ausencia de una diferencia operativamente
	relevante.
	...
	estadounidenses.  Los resultados de este trabajo se sitúan dentro de un rango
	comparable, lo que valida la aplicabilidad del enfoque a la geografía vial de
	La Paz, B.C.S.
	...
	\begin{itemize}
		...
		\item Los tiempos de preparación son homogéneos ($t_{\text{prep}} = 15 \pm 5$~min),
	...
	en función de la relación órdenes/courier.

	\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.85\textwidth]{img/fig_kpi_vs_ratio.png}
	\caption{Evolución de KPIs principales en función de la saturación.}
	...
	satisfactoriamente.  Se ejecutaron 16~corridas de simulación (4~escenarios
	$\times$ 2~semillas $\times$ 2~políticas) sobre la red vial real, y se
	...
	demográfica real de La Paz.  Las dos semillas ($s \in \{2025, 2026\}$) producen
	instancias reproducibles de $\sim$1\,040~órdenes distribuidas en 25~restaurantes
	geolocalizados.

	\paragraph{OE\,2: Integración OSRM--simulador.}
	El motor de simulación de eventos discretos se integró con OSRM v5.26 sobre
	cartografía OpenStreetMap, utilizando el algoritmo MLD para el cálculo de
	tiempos y rutas de viaje reales.  La cobertura del grafo vial se validó
	verificando que el 100\% de los puntos de entrega y recolección obtienen
	...
	Se aplicó la prueba de Mann-Whitney~$U$ al Click-to-Door en los cuatro escenarios,
	obteniendo $p < 0.001$ en todos los casos.  La correlación rank-biserial
	varía de $r = 0.979$ (Grande) en S1 a $r = 0.255$ (Pequeño) en S4,
	confirmando diferencias estadísticamente significativas cuya dirección e
	intensidad dependen del nivel de saturación.

	% ──────────────────────────────────────────────────────────────────
	\section{Verificación de la hipótesis}

	La hipótesis planteaba que la política RH superaría significativamente a FCFS en
	eficiencia operativa y nivel de servicio.  Los resultados la confirman
	\emph{parcialmente}, revelando una dependencia crítica del nivel de saturación:

	\begin{itemize}
		\item \textbf{Baja saturación (S1--S2).}  FCFS es superior en CtD promedio
		      (10.05 vs 29.59~min en S1; 17.03 vs 32.79~min en S2) porque despacha
		      cada pedido inmediatamente sin latencia de agrupamiento.  En este
		      régimen, la hipótesis no se sostiene para la métrica CtD.
		\item \textbf{Saturación media (S3).}  Los tiempos medios convergen
		      (37.11 vs 37.82~min), pero RH comprime la cola derecha: P90~CtD
		      $-53.1\%$ y CTD~Overage $-60.4\%$.  La hipótesis se sostiene
		      para indicadores de robustez del servicio.
		\item \textbf{Alta saturación (S4).}  RH reduce el CtD $-26.3\%$,
		      el P90~CtD $-42.7\%$ y los pedidos no entregados de 12.4\% a 1.2\%.
		      La hipótesis se confirma plenamente en este régimen.
	\end{itemize}

	En cuanto a la eficiencia de la flota, el throughput bajo RH es igual o superior
	en todos los escenarios, alcanzando $+12.8\%$ en S4.  La utilización crece
	consistentemente bajo RH ($+83.6\%$ en S1 a $+3.5\%$ en S4), lo que indica un
	uso más intensivo de los recursos de transporte.
	...
	\begin{enumerate}
		...
		\item \textbf{Evidencia empírica del efecto de la saturación.}  Se documentó
		      ...
		      a superar a FCFS ($\rho \approx 35$, escenario S3), aportando una guía
		      práctica para operadores de \emph{delivery}.
		\item \textbf{Validación sobre topología real.}  Los resultados confirman que
		      las mejoras reportadas por \citet{Reyes2018} en redes euclidianas se
		      mantienen al utilizar distancias reales sobre la red vial de La Paz.
